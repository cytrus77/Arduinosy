...
void Adafruit_INA219::setCalibration(const uint16_t calValue)
{
  ina219_currentDivider_mA = 10;  // Current LSB = 100uA per bit (1000/100 = 10)
  ina219_powerMultiplier_mW = 2;     // Power LSB = 1mW per bit (2/1)

  uint16_t configSet = 0;
  uint16_t configRead = 0;
  wireReadRegister(INA219_REG_CONFIG, &configRead);
  configSet = configRead | 0x7F8;
  Serial.print("INA219_REG_CONFIG READ1: "); Serial.println(configRead);

  do
  {
    wireWriteRegister(INA219_REG_CONFIG, configSet);
    wireReadRegister(INA219_REG_CONFIG, &configRead);
    Serial.print("INA219_REG_CONFIG WRITE: "); Serial.println(configSet);
    Serial.print("INA219_REG_CONFIG READ:  "); Serial.println(configRead);
  } while(configSet != configRead);

  // Set Calibration register to 'Cal' calculated above
  uint16_t calRead = 0;
  wireReadRegister(INA219_REG_CALIBRATION, &calRead);
  Serial.print("INA219_REG_CALIBRATION READ1:  "); Serial.println(calRead);

  do
  {
    wireWriteRegister(INA219_REG_CALIBRATION, calValue);
    wireReadRegister(INA219_REG_CALIBRATION, &calRead);
    Serial.print("INA219_REG_CALIBRATION WRITE: "); Serial.println(calValue);
    Serial.print("INA219_REG_CALIBRATION READ:  "); Serial.println(calRead);
  } while((calValue & 0xFFFE)  != calRead);
}
