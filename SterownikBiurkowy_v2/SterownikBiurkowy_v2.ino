...
String menu[] = {"Swiatlo Male", "Swiatlo Duze"};
String submenu1[] = {"Sila [%]", "Czas [sec]"};
String submenu2[] = {"Sila [%]", "Czas [sec]"};
int value1[sizeof(submenu1)];
int value2[sizeof(submenu2)];
...
int * encoderValue = value1;
int * encoderMax = &valueMax;
int * encoderMin = &valueMin;
...
void loop() {
  ...
  pirTimer ();
  analogWrite(pwmPin, (int)(value2[1]*value2[1]*ledSwitch)/40);
  Serial.println(value2[1]);
  Serial.println((int)(value2[1]*value2[1]*ledSwitch)/40);
  char menu[10] = "Swiatelko";
  char submenu[4] = "Moc";
  LCDDispaly(3, menu, submenu, value2[1]);

}


void LCDDispaly(int pos, String menu, String submenu, int value)
{
  static char txt_temp[16];
  static long value_temp;
  static String menu_temp;
  static String submenu_temp;
  if(value_temp != value && menu_temp != menu && submenu_temp != submenu)
  {
    lcd.clear();
    if(pos==1) { 
      lcd.setCursor(0, 0);
...
void updateEncoder() {
  ...
  if ((sum == 0b1101 || sum == 0b0100 || sum == 0b0010 || sum == 0b1011) && (*encoderValue) > (*encoderMin) ) 
  {
  ...
  if ((sum == 0b1110 || sum == 0b0111 || sum == 0b0001 || sum == 0b1000) && (*encoderValue) < (*encoderMax) ) 
  {
...
void pirTimer () {
  if (millis() - time > Light_Time + PIR_Monitor_Time)
  {
    if (ledSwitch < 1) ledSwitch = ledSwitch + 0.01;
    LCDDispaly(0, "Spie sobie", "", 0);
  }
  else if (millis() - time > Light_Time) 
  {
